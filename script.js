...
function buildNaturalTTS(sortedKorean) {
  ...
  // 결제 키워드는 terminal보다 먼저 처리 (terminal이 섞여도 자연스럽게)
  if (sortedKorean.includes("카드")) {
    const rest = sortedKorean.filter(w => w !== "카드" && !TERMINAL_SET.has(w)).map(getWordTTS);
    return rest.length ? `${rest.join(" ")} 카드로 결제할게요` : "카드로 결제할게요";
  }
  if (sortedKorean.includes("포인트")) {
    const rest = sortedKorean.filter(w => w !== "포인트" && !TERMINAL_SET.has(w)).map(getWordTTS);
    return rest.length ? `${rest.join(" ")} 포인트 할게요` : "포인트 할게요";
  }
  if (sortedKorean.includes("영수증")) {
    const rest = sortedKorean.filter(w => w !== "영수증" && !TERMINAL_SET.has(w)).map(getWordTTS);
